...
// Q) 6
AVLFlightSchedule *sortedList = NULL;
AVLFlightSchedule *appendToFlightSchedule(AVLFlightSchedule *sortedList, AVLFlightSchedule *newflight){
    if (sortedList == NULL)
    {
        return newflight;
    }
    else if (sortedList->flightId > newflight->flightId)
    {
        sortedList->left = appendToFlightSchedule(sortedList->left, newflight);
    }
    else if (sortedList->flightId < newflight->flightId)
    {
        sortedList->right = appendToFlightSchedule(sortedList->right, newflight);
    }
    return sortedList;
}
void searchFlightInRange(AVLFlightSchedule *flight, Time startTime, Time endTime){
    if (flight != NULL)
    {
        ...
        {
            AVLFlightSchedule *newPtr = (AVLFlightSchedule *)malloc(sizeof(AVLFlightSchedule));
            newPtr = flight;
            newPtr->left = NULL;
            newPtr->right = NULL;
            sortedList = appendToFlightSchedule(sortedList, newPtr); //
        }
...
void showRangeHelper(AVLBucket *bucketList, Time startTime, Time endTime){
    if (bucketList != NULL)
    {
        showRangeHelper(bucketList->left, startTime, endTime);
        searchFlightInRange(bucketList->FlightSchedule, startTime, endTime);
        showRangeHelper(bucketList->right, startTime, endTime);
    }
}

void showFlightInRange(AVLBucket *bucketList, Time startTime, Time endTime){
    showRangeHelper(bucketList, startTime, endTime);
     (sortedList);
}
...
//     if (flight != NULL)
//     {
//         if (maxTime(flight->departureTime, startTime) >= 0 && maxTime(endTime, flight->departureTime) >= 0)
//         {
//             printf("Flight %d\n", flight->flightId);
//             printf("Departure Time: %d:%d\n", flight->departureTime.hrs, flight->departureTime.min);
//             printf("ETA: %d:%d\n", flight->ETA.hrs, flight->ETA.min);
